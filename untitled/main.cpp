...
int main(){
    ...
    while(RPS.IsDeadzoneActive()==0){ //haven't pushed it
        ...
        //Case distinction in case we want to aim toward the safe side
            case REDLIGHT:
                pushToward(9.25-awayFromCenter/4.0,-15,.8,0.2);
            break;
            case BLUELIGHT:
                pushToward(9.25+awayFromCenter/4.0,-15,.8,0.2);
            break;
    ...
    while(RPS.IsDeadzoneActive()!=2){ //until activated
        ...
        //Case distinction in case we want to aim toward the safe side
            case REDLIGHT:
                pushToward(9.25-awayFromCenter/4.0,-13,.8,0.2);
            break;
            case BLUELIGHT:
                pushToward(9.25+awayFromCenter/4.0,-13,.8,0.2);
            break;
    ...
    moveComponents(-16,0,1);
    doc("LINE UP FOR LEVER");
    moveTo(wrenchX+1.5,wrenchY,3);  //line up East-West for lever
    doc("LINE UP N/S");
    movementEndTime=TimeNow()+8;
        moveTo(wrenchX-2.5,wrenchY-10,4); //up to lever
    doc("PUSHING LEVER");
